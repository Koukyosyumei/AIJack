...
struct XGBoostNode : public Node<XGBoostParty> {
  ...
  float best_entropy;
  bool use_only_active_party, is_robust;
  ...
  XGBoostNode(vector<XGBoostParty> &parties_, vector<float> &y_,
      ...
      : gradient(gradient_),
        hessian(hessian_), Node<XGBoostParty>(parties_, idxs_, y_) {
  ...
  XGBoostNode &operator=(const XGBoostNode &other) {
    if (this != &other) {
      ...
      for (int i = 0; i < other.parties.size(); i++) {
        parties[i].lookup_table = other.parties[i].lookup_table;
      }
      y = other.y;
      idxs = other.idxs;
      val = other.val;
  ...
  void make_children_nodes(int best_party_id, int best_col_id,
    ...
    left = new XGBoostNode(parties, y, num_classes, gradient, hessian,
                           left_idxs, min_child_weight, lam, gamma, eps,
                           depth - 1, active_party_id, use_only_active_party,
                           n_job, is_robust);
    if (left->is_leaf_flag == 1) {
      left->party_id = party_id;
    }
    right = new XGBoostNode(parties, y, num_classes, gradient, hessian,
