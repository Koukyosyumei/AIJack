...
class WrapperSplitNNNormAttack(WrapperAttacker):
    def __init__(
        self,
        attacker,
        attack_params={},
    ...
    def attack(self):
        ...
        if self.victim_train_dataloaders is not None:
            result["train_leak_auc"] = self.attacker.attack(
                self.victim_train_dataloaders[0], self.victim_criterions[0], self.device
            )
        if self.victim_test_dataloaders is not None:
            result["test_leak_auc"] = self.attacker.attack(
                self.victim_test_dataloaders[0], self.victim_criterions[0], self.device
            )
        return result
