...
PYBIND11_MODULE(aijack_cpp_core, m)
{
    ...
    m.def("eps_gaussian", &eps_gaussian, R"pbdoc(eps_gaussian)pbdoc");

    m.def("eps_laplace", &eps_laplace, R"pbdoc(eps_laplace)pbdoc");

    m.def("eps_randresp", &eps_randresp, R"pbdoc(eps_randresp)pbdoc");

    m.def(
        "calc_tightupperbound_lowerbound_of_rdp_with_theorem6and8_of_zhu_2019",
        &calc_tightupperbound_lowerbound_of_rdp_with_theorem6and8_of_zhu_2019,
        R"pbdoc(calc_tightupperbound_lowerbound_of_rdp_with_theorem6and8_of_zhu_2019)pbdoc");

    m.def("calc_upperbound_of_rdp_with_Sampled_Gaussian_Mechanism",
          &calc_upperbound_of_rdp_with_Sampled_Gaussian_Mechanism,
          R"pbdoc(calc_upperbound_of_rdp_with_Sampled_Gaussian_Mechanism)pbdoc");

    m.def("_ternary_search", &_ternary_search, R"pbdoc(_ternary_search)pbdoc");

    m.def("_ternary_search_int", &_ternary_search_int,
          R"pbdoc(_ternary_search_int)pbdoc");

    m.def("_greedy_search", &_greedy_search, R"pbdoc(_greedy_search)pbdoc");

    m.def("_greedy_search_frac", &_greedy_search_frac,
          R"pbdoc(_greey_search_frac)pbdoc");
    ...
    py::class_<PaillierCipherText>(m, "PaillierCipherText")
        ...
        .def("__add__",
             overload_cast_<PaillierCipherText>()(&PaillierCipherText::operator+))
    ...
    py::class_<XGBoostNode>(m, "XGBoostNode")
        ...
        .def("get_num_parties", &XGBoostNode::get_num_parties)
